...
export abstract class InMemoryRepository<E extends Entity> implements RepositoryInterface<E> {
    ...
    protected async _get(id: string): Promise<E> {
        const response = this.items.find(item => item.id === id)

        if(!response){
            throw new NotFroundError(`Entity not Found using Id ${id}`)
        }
...
export abstract class InMemorySearchabledRepository<E extends Entity>  extends InMemoryRepository<E> implements SearchableRepositoryInterface<E, any, any>{
